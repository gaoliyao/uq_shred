"""
UQ-SHRED Experiment: Flow Over Cylinder Dataset

Trains SHRED (baseline) and UQ-SHRED on flow-over-cylinder data.
Produces: training_curve, calibration, error_vs_unc, timeseries.
...
uq_shred = UQ_SHRED(num_sensors, m, hidden_size=64, hidden_layers=2,
                    l1=350, l2=400, dropout=0.1, noise_dim=500).to(device)
...
# Inverse-transform (test_indices is already sorted for sequential split)
...
# 4. Timeseries at 10 random spatial locations (2 rows × 5 cols, sequential order)
...
T_show = min(100, len(test_truth_np))
...
for row in range(2):
    for col in range(5):
        ...
        ax.plot(times_ts, test_truth_np[:T_show, xi], "k-", lw=1.2, label="Truth")
...
# 5. Spatial snapshots — 4 panels: GT | Median | |Error| | 95% CI Width (2D images, 400×1000)
IMG_H, IMG_W = 400, 1000
t0_snap = test_truth_np[0]
m0_snap = median_orig[0]
e0_snap = np.abs(t0_snap - m0_snap)
w0_snap = (np.percentile(samples_orig, 97.5, axis=0)[0] -
           np.percentile(samples_orig,  2.5, axis=0)[0])
...
im0 = axes[0].imshow(truth_img,  aspect="auto", cmap="coolwarm", vmin=vmin, vmax=vmax)
axes[0].set_title("Ground Truth"); axes[0].axis("off")
plt.colorbar(im0, ax=axes[0], fraction=0.046)
im1 = axes[1].imshow(median_img, aspect="auto", cmap="coolwarm", vmin=vmin, vmax=vmax)
axes[1].set_title("UQ-SHRED Median"); axes[1].axis("off")
plt.colorbar(im1, ax=axes[1], fraction=0.046)
im2 = axes[2].imshow(error_img,  aspect="auto", cmap="Reds")
axes[2].set_title("|Error|"); axes[2].axis("off")
plt.colorbar(im2, ax=axes[2], fraction=0.046)
im3 = axes[3].imshow(width_img,  aspect="auto", cmap="Oranges")
