"""
UQ-SHRED Experiment: Isotropic Turbulence Dataset

Trains SHRED (baseline) and UQ-SHRED on isotropic turbulence data.
...
# Random split: 1000 train, remainder split evenly to valid/test
...
fit(shred, train_dataset, valid_dataset, batch_size=128, num_epochs=1000, lr=1e-3, verbose=True, patience=50)
...
uq_shred = UQ_SHRED(num_sensors, m, hidden_size=64, hidden_layers=2,
                    l1=350, l2=400, dropout=0.1, noise_dim=115).to(device)
uq_errors = fit_uq(uq_shred, train_dataset, valid_dataset,
                   batch_size=128, num_epochs=1000, lr=1e-3, verbose=True, patience=50)
...
for row, t in enumerate(snap_ts):
    ...
    truth_img  = t_snap.reshape(IMG_ISO, IMG_ISO)
    median_img = m_snap.reshape(IMG_ISO, IMG_ISO)
    error_img  = e_snap.reshape(IMG_ISO, IMG_ISO)
    width_img  = w_snap.reshape(IMG_ISO, IMG_ISO)
