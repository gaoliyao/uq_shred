...
# 6. Spacetime plot: x=sample, y=neuron (4 panels: GT | Median | |Error| | Std)
err_st = np.abs(test_truth_np - median_orig)
fig, axes = plt.subplots(1, 4, figsize=(20, 5))
fig.suptitle("UQ-SHRED: Spacetime Plot", fontsize=14, fontweight="bold")
im0 = axes[0].imshow(test_truth_np.T, aspect="auto", cmap="RdBu_r", interpolation="nearest")
axes[0].set_title("Ground Truth"); axes[0].set_xlabel("Sample"); axes[0].set_ylabel("Neuron")
axes[0].set_yticks(range(m)); axes[0].set_yticklabels(REGION_LABELS, fontsize=6)
plt.colorbar(im0, ax=axes[0], fraction=0.046)
im1 = axes[1].imshow(median_orig.T, aspect="auto", cmap="RdBu_r", interpolation="nearest")
axes[1].set_title("UQ-SHRED Median"); axes[1].set_xlabel("Sample")
axes[1].set_yticks(range(m)); axes[1].set_yticklabels(REGION_LABELS, fontsize=6)
plt.colorbar(im1, ax=axes[1], fraction=0.046)
im2 = axes[2].imshow(err_st.T, aspect="auto", cmap="Reds", interpolation="nearest")
axes[2].set_title("|Error|"); axes[2].set_xlabel("Sample")
axes[2].set_yticks(range(m)); axes[2].set_yticklabels(REGION_LABELS, fontsize=6)
plt.colorbar(im2, ax=axes[2], fraction=0.046)
im3 = axes[3].imshow(std_orig.T, aspect="auto", cmap="Oranges", interpolation="nearest")
axes[3].set_title("Predicted Std"); axes[3].set_xlabel("Sample")
axes[3].set_yticks(range(m)); axes[3].set_yticklabels(REGION_LABELS, fontsize=6)
plt.colorbar(im3, ax=axes[3], fraction=0.046)
plt.tight_layout()
fig.savefig(results_dir / "uq_shred_spacetime.png", dpi=150, bbox_inches="tight")
print(f"  Saved: {results_dir}/uq_shred_spacetime.png")
plt.close(fig)
